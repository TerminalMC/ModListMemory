<!--suppress HtmlDeprecatedAttribute, HtmlDeprecatedTag, XmlDeprecatedElement -->
<div align="center"><center>

<img alt="Icon" width=100 src="https://raw.githubusercontent.com/TerminalMC/ModListMemory/HEAD/common/src/main/resources/assets/modlistmemory/icon.png">

## Mod List Memory

Moves recently-opened mods to the top of the mod list, or remembers your scroll position.

Works with [ModMenu](https://modrinth.com/project/mOgUt4GM) on Fabric and
[Better Modlist](https://modrinth.com/project/sbpqhzIG) on NeoForge.

[![Environment](https://img.shields.io/badge/Environment-Client-blue?logo=data:image/png;base64,)]()
[![Latest Minecraft](https://img.shields.io/modrinth/game-versions/qap5C41G?label=Latest%20Minecraft&color=%2300AF5C&logo=data:image/png;base64,)](https://modrinth.com/project/qap5C41G/versions)

[![Loader](https://img.shields.io/badge/Available%20for-Fabric-dbd0b4?logo=data:image/png;base64,)](https://fabricmc.net/)
[![Loader](https://img.shields.io/badge/Available%20for-Forge%201.20.1-1d2d41?logo=data:image/png;base64,)](https://files.minecraftforge.net)
[![Loader](https://img.shields.io/badge/Available%20for-NeoForge%201.21%2B-f16436?logo=data:image/png;base64,)](https://neoforged.net/)

[![Available on Modrinth](https://img.shields.io/modrinth/dt/qap5C41G?label=Available%20on%20Modrinth&logo=modrinth&logoColor=%2300AF5C)](https://modrinth.com/project/qap5C41G)
[![Available on CurseForge](https://img.shields.io/curseforge/dt/1180596?label=Available%20on%20CurseForge&logo=curseforge)](https://curseforge.com/minecraft/mc-mods/modlistmemory)
[![Available on GitHub](https://img.shields.io/github/downloads/TerminalMC/ModListMemory/total?label=Available%20on%20GitHub&logo=github&logoColor=white)](https://github.com/TerminalMC/ModListMemory)

</center></div>

### Compatibility

- If you encounter issues with ModListMemory, please first check that you are using the correct version to match the
  installed version of your mod-list mod, if there is any. You can check this by looking at the last `x.x.x` number in
  the mod version, e.g. `v0.2.1+1.21+1.1.13` indicates that the version is compiled against Better Modlist `v1.1.13`.
- If the issue persists on the correct version, or if you are using the latest version of your mod-list mod and there is
  not yet a version of ModListMemory to match, please report the issue via [Discord](https://discord.terminalmc.dev) or
  [GitHub](https://github.com/TerminalMC/ModListMemory/issues).

### Features

<details open>
<summary><b>Memory</b></summary>

- `Remember Recent` records when you open mods from the list, and moves those mods to the top of the list.
- `Remember Scroll` saves the scrollbar position and restores it when you reopen the mod list.
</details>


<details open>
<summary><b>Pinning</b></summary>

- Hold a modifier key (default: `Shift`) while clicking on a mod from the list to pin that mod to the top of the list.
- Re-order pins by holding the same modifier key while clicking on a pinned mod.
- Unpin mods by holding the other modifier key (default: `Control`) while clicking on a pinned mod.
</details>


<details open>
<summary><b>Persistence</b></summary>

- `Persist on restart` makes the mod save its memory to the config file, allowing it to be restored when you restart
  the game.
- `Save on update` makes the mod save its memory whenever you open a mod from the list, rather than only when you quit
  the game. This takes a small amount of time (~1ms with a memory size of 5) and allows
  `Persist on restart` to work even if the game crashes or otherwise fails to shut down properly.
</details>

### Dependencies

- [ModMenu](https://modrinth.com/project/mOgUt4GM) and [Fabric API](https://modrinth.com/project/P7dR8mSH) on Fabric
- [Better Modlist](https://modrinth.com/project/sbpqhzIG) on Neo/Forge
- [Cloth Config API](https://modrinth.com/project/9s6osm5g) for configuration

### Contact

[![Discord](https://img.shields.io/discord/1103153365216669797?logo=discord&label=Discord%20Server&color=%235865F2)](https://discord.terminalmc.dev)

[![Issues](https://img.shields.io/github/issues/TerminalMC/ModListMemory?logo=github&label=GitHub%20Issues)](https://github.com/TerminalMC/ModListMemory/issues)

[![License](https://img.shields.io/github/license/TerminalMC/ModListMemory?label=License&logo=github&logoColor=white)](https://github.com/TerminalMC/ModListMemory/blob/HEAD/LICENSE.txt)
